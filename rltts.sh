...
test_domain() {
    ...
    while [ $attempt -lt $RETRY_TIMES ]; do
        o=$(curl -s -v $stack_flag --connect-timeout "$CONNECT_TIMEOUT" --max-time "$MAX_TIME" --tlsv1.3 --tls-max 1.3 \
                "${curl_extra[@]}" \
                -I -o /dev/null \
    ...
    if [ -z "$h" ] || [ "$h" = "0.000000" ]; then
        echo "5|$label|失败|-|-|-|-|-|9999|-|$sni|-" >> "$outfile"
...
^< x-cache:.*(hit|miss)|^< via:.*(cdn|varnish)"; then
    ...
    if [ "$curl_exit" -eq 0 ] && [ "$http_responded" -eq 1 ]; then
        ...
        for ((i = 1; i < SAMPLE_TIMES; i++)); do
            hs=$(curl -s -I -o /dev/null $stack_flag --connect-timeout "$CONNECT_TIMEOUT" --max-time "$MAX_TIME" \
                    --tlsv1.3 --tls-max 1.3 "${curl_extra[@]}" -w "%{time_appconnect}" "$curl_url" 2>/dev/null)
    ...
    # ---------- 最终判定：只把真正的硬条件作为淘汰 ----------
    # 3=不合格：已经完成连接，但 TLS 不是1.3、明确发生跳转、证书明确失效。
    # 1=可用/警告：TLS1.3 已建立，但 H2/CDN/X25519/地区等存在风险或未知。
    # 5=连接失败：该协议栈本次无法完成握手，不代表另一个协议栈也失败。
    # 4=该协议栈无DNS记录。
    ...
    if [ "$t" != "TLSv1.3" ]; then
        ...
        # TLS1.3 已建立后，以下项目全部只做质量/风险提示，不一票否决。
        if [ "$CURVES_SUPPORTED" -eq 1 ]; then
            ...
            for ((x_try = 0; x_try < X25519_RETRY_TIMES; x_try++)); do
                if curl -s -o /dev/null $stack_flag --connect-timeout "$CONNECT_TIMEOUT" --max-time "$MAX_TIME" \
                        --tlsv1.3 --tls-max 1.3 --curves X25519 "${curl_extra[@]}" -I "$curl_url" >/dev/null 2>&1; then
                    x_ok=0
                    break
                fi
            done
...
while true; do
    ...
    if [ "$d" = "0" ]; then
        echo -e "\033[1;32m[OK] 已退出，不修改命令历史或系统缓存。\033[0m"
        break
    fi
    ...
    sort -t'|' -k1,1n -k9,9n "$result_file" | while IFS='|' read -r status dom tls alpn cf redirect x25519 cert hs ip sni pq; do
        case "$status" in
            ...
            5) color="\033[90m"; result_label="连接失败" ;;
    ...
    echo -e "\033[32m■\033[0m 推荐   \033[1;33m■\033[0m 硬条件通过但有警告   \033[90m■\033[0m 不合格   \033[90m■\033[0m 连接失败   \033[36m■\033[0m 无DNS记录"
    echo -e "\033[2m  硬条件：必须能建立 TLS1.3；明确 301/302/303/307/308 跳转；证书明确失效。CDN 不再硬性淘汰。\033[0m"
